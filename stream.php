...
setcookie("errcode", ""); //EventSource Fehlermeldung kann nicht abgerufen werden, wird über Cookie übergeben
setcookie("errmsg", "");
...
$callback = function ($ch, $data) {
    ...
    if (isset($complete->error)) {
        ...
        if (strpos($complete->error->message, "Rate limit reached") === 0) { //Fehler bei Überschreitung der Zugriffsfrequenz gibt einen Nullcode zurück, Sonderbehandlung
            setcookie("errcode", "rate_limit_reached");
        }
        if (strpos($complete->error->message, "Your access was terminated") === 0) { //Verstoß gegen die Nutzungsbedingungen, Verbot, Sonderbehandlung für eine gewisse Zeit
        ...
        if (strpos($complete->error->message, "You exceeded your current quota") === 0) { //Niedriger API-KEY-Saldo
            setcookie("errcode", "insufficient_quota");
        }
        if (strpos($complete->error->message, "That model is currently overloaded") === 0) { //OpenAI-Server überlastet
